...
from .commands import PAID_USERS
...
async def run(bot, message):
    ...
    if fromid.text and not fromid.forward_date:
        ...
        regex = re.compile("(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)(/(\d+))?$")
        ...
        last_msg_id = int(match.group(7) or match.group(5))
        ...
        if last_msg_id == None:
            return await message.reply_text(
                "**This may be a forwarded message from a group and sended by anonymous admin. instead of this please send last message link from group**"
            )
    ...
    STS(forward_id).store(chat_id, toid, int(skipno.text), int(last_msg_id), thread_id)

# AUTO TOPIC PATCH READY
